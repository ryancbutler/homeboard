...
type Template = {
  id: string; household_id: string; title: string; assignment_policy: "individual" | "any" | "every";
  approval_required: boolean; schedule_kind: Schedule["kind"]; start_date: string; due_time: string | null; weekdays: number[];
};

export async function materializeChores(until = new Date(Date.now() + 30 * 86_400_000)) {
  const templates = await db<Template[]>`
    SELECT ct.*, COALESCE(ct.weekdays, '{}') AS weekdays
    FROM chore_templates ct WHERE ct.active = true`;
  for (const template of templates) {
    const [{ timezone }] = await db<{ timezone: string }[]>`SELECT timezone FROM households WHERE id = ${template.household_id}`;
    const today = dateInTimezone(new Date(), timezone);
    const finalDay = dateInTimezone(until, timezone);
    const schedule: Schedule = { kind: template.schedule_kind, startDate: template.start_date, weekdays: template.weekdays, dueTime: template.due_time };
    const assignees = await db<{ member_id: string }[]>`SELECT member_id FROM chore_template_assignees WHERE chore_template_id = ${template.id}`;
    for (const day of scheduledDays(schedule, today, finalDay)) {
      const occurrence = await db<{ id: string }[]>`
        ...
        VALUES (${template.id}, ${template.household_id}, ${day}, ${dueAt(day, template.due_time, timezone)})
      ...
      if (template.assignment_policy === "every") {
        for (const assignee of assignees) await db`
        ...
        await db`
          INSERT INTO chore_obligations (occurrence_id, member_id, approval_status)
          SELECT ${occurrenceId}, ${template.assignment_policy === "individual" ? assignees[0]?.member_id ?? null : null}, 'not_required'
...
export async function materializeRoutines(until = new Date(Date.now() + 30 * 86_400_000)) {
  const templates = await db<{
    id: string; household_id: string; assignment_policy: "individual" | "any" | "every";
    schedule_kind: Schedule["kind"]; start_date: string; due_time: string | null; weekdays: number[];
  ...
  for (const template of templates) {
    ...
    const today = dateInTimezone(new Date(), timezone);
    const schedule: Schedule = { kind: template.schedule_kind, startDate: template.start_date, weekdays: template.weekdays, dueTime: template.due_time };
    const assignees = await db<{ member_id: string }[]>`SELECT member_id FROM routine_template_assignees WHERE routine_template_id = ${template.id}`;
    ...
    for (const day of scheduledDays(schedule, today, dateInTimezone(until, timezone))) {
      const owners = template.assignment_policy === "every" ? assignees.map((value) => value.member_id) : [template.assignment_policy === "individual" ? assignees[0]?.member_id ?? null : null];
      for (const owner of owners) await db`
