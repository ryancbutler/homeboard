...
function ParentContent() {
  ...
  if (!authenticated) {
    return (
      <main className="parent-shell pin-lock-shell">
  ...
  return (
    <main className="parent-shell">
      ...
      <section className="family-pulse" aria-labelledby="family-pulse-title">
          ...
          {pending.length > 0 && (
            <div className="needs-attention" role="status">
              ...
              <div>
                ...
                <strong>
                  {pending.length} approval{pending.length === 1 ? "" : "s"} to review
                </strong>
              </div>
            </div>
          )}
        ...
        {!familyToday ? (
          <p className="form-hint">Loading today&apos;s family picture…</p>
        ) : (
          <>
            ...
            {familyToday.sharedOpenChores > 0 && (
              <p className="shared-work-callout">
                <Home size={18} aria-hidden="true" />
                {familyToday.sharedOpenChores} shared or unassigned chore{familyToday.sharedOpenChores === 1 ? "" : "s"}{" "}
                still open
              </p>
            )}
          </>
          ...
          <>
            <div className="reschedule-dialog-head">
              <div>
                ...
                <h2 id="reschedule-title">
                  {rescheduleDialog.kind === "postpone" ? "Postpone" : "Reschedule"} {rescheduleDialog.title}
                </h2>
              </div>
              <button
                type="button"
                ...
                onClick={dismissReschedule}
              >
                <X size={18} aria-hidden="true" />
              </button>
            </div>
            <p id="reschedule-description" className="reschedule-copy">
              {rescheduleDialog.kind === "postpone"
                ? `Choose a later day through Sunday for ${rescheduleDialog.child ?? "this chore"}. Its due time carries over.`
            ...
            {!rescheduleDialog.days ? (
              <p className="form-hint" role="status">
                Checking this week&apos;s available days…
              </p>
            ) : (
              <div className="reschedule-days" aria-label="Available days through Sunday">
                {rescheduleDialog.days.map((day) => (
                  <button
                    key={day.date}
                    type="button"
                    className={`reschedule-day${rescheduleDialog.selectedDate === day.date ? " selected" : ""}`}
                    disabled={!day.available || Boolean(busy)}
                    aria-pressed={rescheduleDialog.selectedDate === day.date}
                    onClick={() =>
                      setRescheduleDialog((current) => (current ? { ...current, selectedDate: day.date } : current))
                    }
                  >
                    <span>{day.label}</span>
            ...
            <div className="reschedule-actions">
              <button type="button" className="secondary" disabled={Boolean(busy)} onClick={dismissReschedule}>
                Cancel
              </button>
              <button
                type="button"
                className="primary"
                disabled={!rescheduleDialog.selectedDate || Boolean(busy)}
                onClick={confirmReschedule}
              >
                <RefreshCw size={16} aria-hidden="true" />
                {busy?.startsWith(`${rescheduleDialog.kind}-`)
                  ? rescheduleDialog.kind === "postpone"
                    ? "Postponing…"
                    : "Rescheduling…"
                  : rescheduleDialog.kind === "postpone"
