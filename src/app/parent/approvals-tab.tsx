      ...
      <section className="management-card approval-card">
        <div className="history-head">
          <div>
            ...
            <h2>
              Parent approvals <span className="count-badge">{pending.length}</span>
            </h2>
          </div>
          <button
            className="secondary"
        ...
        {!dashboard ? (
          <p className="form-hint">Loading approvals…</p>
        ) : !pending.length ? (
          <p className="form-hint">All caught up. Finished chores that need your approval will appear here.</p>
        ) : (
          pending.map((chore) => {
            const completedByName =
              members.find((member) => member.id === chore.completedBy)?.displayName ??
              chore.assignee?.name ??
              "a teammate";
            const completedTime = chore.completedAt
              ? new Intl.DateTimeFormat("en-US", {
                  hour: "numeric",
                  minute: "2-digit",
                  timeZone: dashboard.household.timezone,
              ...
              <article className="approval-item" key={chore.obligationId}>
                ...
                <div className="approval-actions">
                  <button
                    className="secondary"
                    disabled={Boolean(busy)}
                    onClick={() => review(chore.obligationId, "reject")}
                  >
                    Try again
                  </button>
                  <button
                    className="primary"
                    disabled={Boolean(busy)}
                    onClick={() => review(chore.obligationId, "approve")}
                  >
                    <Check size={17} aria-hidden="true" />
                    {busy === chore.obligationId ? "Saving…" : "Approve"}
                  </button>
                </div>
              </article>
            );
      ...
      <section className="management-card history">
        ...
        <div className="history-toolbar">
          ...
          <select
            ...
            onChange={(e) => {
              setHistoryChildFilter(e.target.value);
              setHistoryPage(1);
            }}
          >
          ...
          <select
            ...
            value={historySortBy}
            onChange={(e) => {
              setHistorySortBy(e.target.value as "date" | "chore" | "child" | "status");
              setHistoryPage(1);
            }}
          >
              ...
              {paginatedHistory.map((row, index) => {
                ...
                return (
                  <tr key={row.obligation_id || `${row.history_date}-${index}`}>
                    ...
                    <td>
                      <span className="history-schedule-tag">{formatHistorySchedule(row)}</span>
                    </td>
                    ...
                    <td>
                      {row.obligation_id &&
                        (row.status === "completed" || row.status === "pending" || row.status === "rejected") && (
                          <button
                            type="button"
        ...
        {filteredHistory.length > 0 && (
          <div className="history-pagination" aria-label="Chore history pagination">
            <span>
              ...
              {Math.min(historyPage * historyPageSize, filteredHistory.length)} of {filteredHistory.length}
            </span>
            <label>
              Rows per page
              <select
